...
class RegisterPage : AppCompatActivity() {
   ...
   auth= FirebaseAuth.getInstance()
        ...
        RegisterBtn.setOnClickListener{
            ...
            var uage = UserAge.text.toString()


            if (!TextUtils.isEmpty(uname) &&!TextUtils.isEmpty(upass) && !TextUtils.isEmpty(umail) && !TextUtils.isEmpty(uage) ){
                ...
                auth.createUserWithEmailAndPassword(umail,upass).addOnCompleteListener(this){
                    if (it.isSuccessful){
                     ...
                     var uid=auth.currentUser?.uid
                        if (uid !=null){
                            var obj1=UserModel(uname,umail,upass,auth.currentUser?.uid )
